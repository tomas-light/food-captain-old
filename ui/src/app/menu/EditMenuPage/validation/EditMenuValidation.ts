...
export class EditMenuValidation extends ValidatorBase implements IValidator<EditMenuFormValues> {
	...
	authorIsNotValid(author: any): boolean {
		return !ValidatorBase.isNullOrUndefined(author) && !ValidatorBase.isPositiveOrZeroNumber(author);
	}

	dishesIsNotValid(dishes: any): boolean {
		return !ValidatorBase.isNullOrUndefined(dishes) && !ValidatorBase.isNumberArray(dishes);
	}
}
