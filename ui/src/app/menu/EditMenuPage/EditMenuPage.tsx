...
const EditMenuPage = (props: Props) => {
	...
	const [Form] = useForm<EditMenuFormValues>(onSave, new EditMenuValidation(t), {
			...
			{(state) => (
				<div className={classes.root}>
					...
					{!openedMenu.createDate ? null : (
						<Typography>{t('create date', { date: openedMenu.createDate.toLocaleDateString() })}</Typography>
					)}

					{!openedMenu.lastUpdate ? null : (
						<Typography>{t('last update', { date: openedMenu.lastUpdate.toLocaleDateString() })}</Typography>
					)}
					{/*<TextFormField
