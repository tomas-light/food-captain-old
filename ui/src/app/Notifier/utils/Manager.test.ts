...
test('filterKeys', () => {
	...
	expect(Manager['snackbarKeys']).toEqual(['k_3', 'k_5']);
});
...
describe('addNewNotifications', () => {
	...
	beforeEach(() => {
		...
		manager = new Manager(mockFn, closeSnackbar, mockFn);
	...
	test('add 1', () => {
		...
		const notifications: Notification[] = [new Notification('mess 1')];
...
describe('enqueueIfNeeded', () => {
	...
	test('new key', () => {
		...
		const manager = new Manager(enqueueSnackbar, closeSnackbar, onExited);
		const notifications: Notification[] = [new Notification('mess')];
