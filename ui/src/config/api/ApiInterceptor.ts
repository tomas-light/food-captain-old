...
import { NotificationImpl } from '~Notifier/Notification';
...
export class ApiInterceptor {
	constructor(private readonly dispatch: Dispatch<Action>) {
		axios.interceptors.request.use(ApiInterceptor.requestFulfilled, ApiInterceptor.requestRejected);

		axios.interceptors.response.use(ApiInterceptor.responseFulfilled as any, this.responseRejected.bind(this));
	}
	...
	private showErrorNotification(error: Error) {
		const message = error ? error.message : SOMETHING_WRONG_MESSAGE;

		const notification = new NotificationImpl({ message, variant: 'warning' });
		this.dispatch(NotifierController.notify(notification));
	}
