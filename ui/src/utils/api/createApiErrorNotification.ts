...
export function createApiErrorNotification(response: ApiResponse): Notification {
	...
	if (response.hasClientError()) {
		return new Notification(response.error, { variant: 'warning' });
	}

	if (response.hasServerError()) {
		return new Notification(response.error, { variant: 'error' });
	}
...
function makeDefaultNotification() {
	return new Notification(SOMETHING_WRONG_MESSAGE, { variant: 'default' });
}
