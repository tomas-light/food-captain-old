...
import { FieldError, FieldLabel, FieldLoadingIndicator } from '@shared/molecules/fields';
		...
		<div className={clsx(_classes.root, classes.root.root)} {...rest}>
			...
			<div className={clsx(_classes.control, classes.root.control)}>
				...
				{Boolean(customEndAdornment) && (
					<EndAdornment classes={classes.endAdornment}>{customEndAdornment}</EndAdornment>
				)}

				<FieldLoadingIndicator visible={isLoading} right={17} {...LoadingIndicatorProps} classes={classes.indicator} />
			</div>

			<FieldError id={helperTextId} error={error} text={helperText} {...ErrorProps} classes={classes.error} />
		</div>
	);
};
