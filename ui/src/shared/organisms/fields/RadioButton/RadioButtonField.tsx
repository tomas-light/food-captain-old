...
import { FormControlLabel, Radio, RadioGroup, StyledComponentProps, createStyles } from '@shared/reexport';
import { withStyles } from '@shared/theme';

import { RadioButtonItem } from './RadioButtonItem';

type RadioButtonFieldClassKey = 'root' | 'label' | 'checked' | 'disabled' | 'radio';

const styles = (theme) =>
	createStyles<RadioButtonFieldClassKey, any>({
		root: {},
		label: {
...
const RadioButtonField = (props: Props) => {
	const { classes, disabled, items, name, value, ...rest } = props;
			...
			{items.map((item: RadioButtonItem) => {
				...
				return (
					<FormControlLabel
						...
						control={<Radio disabled={_disabled} color={'secondary'} className={classes.radio} />}
						label={item.label}
						value={item.id}
						disabled={_disabled}
						classes={{
