...
import { StyledComponentProps } from '@shared/reexport';
...
type ClassKey = 'root' | 'idle' | 'error' | 'dragging';
...
type Props = DragAndDropProps & DragAndDropCallProps & StyledComponentProps<ClassKey>;

const DragAndDrop: FC<Props> = (props) => {
	const { classes, onDrop, children, id = guid(), name, error, fileTypes = 'image/jpeg', ...rest } = props;
	...
	return (
		<div
			...
			{...rest}
		>
			<input id={id} name={name} style={{ display: 'none' }} type="file" accept={fileTypes} onChange={handleOnChange} />
			{children}
		</div>
	);
};
...
export type { DragAndDropProps, DragAndDropCallProps, ClassKey as DragAndDropClassKey };
